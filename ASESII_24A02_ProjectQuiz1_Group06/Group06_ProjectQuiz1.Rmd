...
The exploratory analysis in this section uses only the training observations to avoid information leakage from the holdout set.

### Response distribution

The response variable (`lpsa`) is approximately symmetric, with the mean close to the median. Most observations are concentrated between 1.5 and 3.5, while only a few observations appear in the tails. The histogram and Q–Q plot indicate that the Gaussian assumption is broadly reasonable, although slight departures from the reference line are present in the upper tail.

### One correlation table or focused relationship plot

The correlation heatmap shows that **lcavol** has the strongest positive correlation with the response (`r = 0.75`), indicating that tumor volume is the most informative individual predictor of PSA level. Moderate positive correlations are also observed for **lcp** (`r = 0.59`) and **svi** (`r = 0.58`), whereas `age`, `lbph`, and `lweight` exhibit relatively weak marginal relationships with the response.

### One possible anomaly or influential observation

The boxplots identify several potential outliers, particularly for `age` and `pgg45`, with `pgg45` exhibiting the greatest variability among the predictors. These observations may have a greater influence on the fitted regression model than typical observations. However, because there is no evidence that they arise from data entry errors, they are retained and their influence will be assessed using formal regression diagnostics in the OLS analysis.

### One pair or group of predictors for which multicollinearity may matter

The strongest correlations among the predictors are observed between `gleason` and `pgg45` (`r = 0.76`), `lcavol` and `lcp` (`r = 0.73`), `lcp` and `svi` (`r = 0.67`), and `lcavol` and `svi` (`r = 0.59`). These predictor pairs appear to contain overlapping information, suggesting that multicollinearity may be present. Consequently, ordinary least squares coefficient estimates may become unstable and exhibit inflated variance, motivating the use of ridge and lasso regression to improve coefficient stability and variable selection.

### Question for regularized models

Based on the exploratory analysis, the following question will guide the remaining analysis:

> **Can ridge regression reduce the instability caused by multicollinearity, and can lasso identify a smaller subset of predictors while maintaining comparable predictive performance?**
